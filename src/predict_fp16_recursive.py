...
def parse():
    ...
    parser.add_argument('-ri', type=pu.int_type, 
                        default=5,
                        help='Number of recursive iterations for prediction matrix')
...
def main():
    ...
    with torch.no_grad():
        ...
        for batch_idx, (fixed_batch, moving_batch, loc) in enumerate(prediction_loader):
            ...
            lastRow = torch.Tensor([0, 0, 0, 1]*fixed_batch.shape[0]).to(device)
            lastRow = lastRow.view(-1, 1, 4)
            
            known_batch = known_affine(fixed_batch, device)
            
            predicted_theta = model(fixed_batch, moving_batch)
            warped_batch = affine_transform(moving_batch, predicted_theta)
            
            for i in range(args.ri):
                
                # Predict theta from fixed and warped
                predicted_theta_i = model(fixed_batch, warped_batch)
                
                predicted_theta = torch.matmul(torch.cat((predicted_theta, lastRow), dim=1),
                                               torch.cat((predicted_theta_i, lastRow), dim=1))
                
                # Apply accumulated theta
                warped_batch = affine_transform(moving_batch, predicted_theta)
                
            predicted_theta = predicted_theta[:, 0:3, :]
            warped_batch = affine_transform(moving_batch, predicted_theta_i)
            ...
            accP += print_patchloss(preWarpNcc, postWarpNcc)
       
        print('Average percentwise improvement: ', accP/fixed_patches.shape[0])
...
def known_affine(fixed_batch, device):
    angle = 0 * 3.1415 / 180
   
    batch_theta = torch.Tensor([[math.cos(angle), 0, math.sin(angle), 0], 
                                [0, 1, 0, 0], 
                                [-math.sin(angle), 0, math.cos(angle), 0]]*fixed_batch.shape[0])
    batch_theta = batch_theta.view(-1, 3, 4).to(device)
        
    known_batch = affine_transform(fixed_batch, batch_theta)
